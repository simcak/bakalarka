\chapter{Vlastní algoritmus založený na detekci vrcholů}
\label{ch:VlastniAlg}
...
Oproti tomu databáze \emph{BUT~PPG} používá formát \acl{WFDB} (\acs{WFDB}) a obsahuje \acs{PPG} záznamy v \texttt{.dat} a \texttt{.hea} souborech.
...
Pro odstranění fázového posunu, který je charakteristický pro použitý filtr, jsme použili filtr s nulovou fází, jenž provádí filtraci v obou směrech.
...
Na rozdíl od modusové a průměrné hodnoty byl medián nejméně citlivý na extrémní hodnoty.
